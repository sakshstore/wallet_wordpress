...
$res=( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}aistore_wallet_widthdrawal_requests ( amount,username,currency  ) VALUES ( %s, %s, %s)", array(  $amount, $username,$aistore_currency ) ) );
...
$user_id=get_current_user_id();
 ...
 $body="Hello, <br>
   ...
   <table>
    ...
    <tr><td>".$user->user_bank_detail."</td></tr>
...
$user_id=get_current_user_id();
    $user = get_user_by( 'id', $user_id);
    // echo $user->user_bank_detail;
    if($user->user_bank_detail==="NULL"){
...
<table>
    <tr><td>Bank Details :</td></tr>
    <tr><td><?php echo esc_attr($user->user_bank_detail); ?></td></tr>
    
    <tr><td>Deposit Instructions :</td></tr>
    <tr><td><?php echo esc_attr($user->user_deposit_instruction); ?></td></tr>
</table>
<?php
}
?>
...
global $wpdb;
  ...
  $results = $wpdb->get_results( 
                     $wpdb->prepare("SELECT * FROM {$wpdb->prefix}aistore_wallet_widthdrawal_requests WHERE username=%s order by id desc limit 100", $current_user_email_id) 
