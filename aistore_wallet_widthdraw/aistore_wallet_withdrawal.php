...
global  $wpdb;
 ...
 $results = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}aistore_wallet_widthdrawal_requests");
...
$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}aistore_wallet_widthdrawal_requests
    SET status = '%s'  WHERE id = '%d'", 
   'Approved' , $withdrawal_id   ) );
   
   $widthdrawal = $wpdb->get_row($wpdb->prepare( "SELECT * FROM {$wpdb->prefix}aistore_wallet_widthdrawal_requests WHERE id=%s ",$withdrawal_id));
