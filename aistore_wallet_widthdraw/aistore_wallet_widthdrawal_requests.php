  ...
  <option value="<"><?php  _e( 'Less than', 'aistore' ) ?> </option>
   ...
   <option value="<="><?php  _e( 'Less than or equal', 'aistore' ) ?></option> 
	...
	public static function get_withdrawal( $per_page = 5, $page_number = 1 ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}aistore_wallet_widthdrawal_requests  WHERE 1=1 ";
			}
		...
		$sql = "SELECT * FROM {$wpdb->prefix}aistore_wallet_widthdrawal_requests";
}
...
{ $sql .= " and   ". $search_column."  ".$search_operator." '". $search ."'  ";
	...
	public static function delete_withdrawal( $id ) {
		...
		$wpdb->delete(
			"{$wpdb->prefix}aistore_wallet_widthdrawal_requests",
			[ 'ID' => $id ],
			[ '%d' ]
		);
	}
	...
	public static function record_count() {
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}aistore_wallet_widthdrawal_requests where 1 =1   ";
...
$hook = add_submenu_page( 'wallet_account' ,'Wallet Account', 'All Withdrawal List', 'manage_options', 'withdrawal_list',[ $this, 'plugin_settings_page' ]);
// 		$hook = add_menu_page(
// 			'All Withdrawal List',
// 			'Withdrawal List',
// 			'manage_options',
